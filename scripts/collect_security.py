...
def score_security(r):
    ...
    hype = any(k in desc for k in ["最强","终极","神器","无敌","史上最"])  # 营销话术视为风险信号
    ...
    # 供应链信号 20%（基于可用元数据的代理指标，非代码级扫描）
    # 官方/知名组织供应链可信度高；含营销话术视为风险信号
    if owner in OFFICIAL: s2=90
    elif owner in KNOWN: s2=80
    elif hype: s2=20
    else: s2=65
...
def gen_html(repos,month):
    ...
    return f"""<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><meta name="description" content="AI Agent Skill 安全分级榜单 Top 30。基于出品方可信度、供应链信号、文档质量等 6 维评估，帮你判断哪些 Skill 可以放心安装。每月 3 号更新。"><meta property="og:title" content="AI Agent Skills 安全分级榜单 Top 30 — {month}"><meta property="og:description" content="S 级闭眼装 / A 级放心装 / D 级不建议。6 维安全评估，每月自动更新。"><meta name="twitter:card" content="summary_large_image"><link rel="icon" type="image/svg+xml" href="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32'><rect width='32' height='32' rx='6' fill='%23534AB7'/><text x='16' y='23' text-anchor='middle' font-size='16' font-weight='bold' fill='white' font-family='system-ui'>AS</text></svg>">
...
<div class="footer"><p>• 6维评估：出品方(25%)+供应链信号(20%)+SKILL.md质量(15%)+维护(15%)+社区审查(15%)+安全记录(10%)</p><p>• 参考：OWASP Agentic Skills Top 10 · NVIDIA SkillSpector (26.1%含漏洞)</p><p>• 供应链信号为基于出品方/描述营销话术的代理指标，非代码级扫描；仅供参考，不构成安全建议 · collect_security.py v1.1 · {now}</p></div>
</div></body></html>"""
